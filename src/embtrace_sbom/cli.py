"""CLI entry point for embtrace-sbom (standalone CRA Readiness Check collector).

Usage:
    embtrace-sbom .                          # read the build, write your SBOM — sends NOTHING
    embtrace-sbom . --send --email you@example.com   # free CRA report (asks first)
    embtrace-sbom . --sbom bill.json         # write the SBOM somewhere else
...
@click.version_option(version=__version__, prog_name="embtrace-sbom")
@click.argument(
    "path",
    ...
    *,
    ...
    if uploading and not code and "@" not in (contact_email or ""):
        console.print(
            ...
            "is sent to). Example: embtrace-sbom . --send --email "
            "you@example.com"
        )
        sys.exit(1)
    ...
    if not real:
        ...
        if stats.build_files_scanned == 0:
            ...
            console.print(
                ...
                "-DWITH_SSL=ON`) so embtrace-sbom reads which you actually "
...
def main_check_alias() -> None:
    """``embtrace-check`` — the old command name, kept so printed instructions
    and today's articles keep working. One line of notice, then the same tool."""
    Console(stderr=True).print(
        "[yellow]embtrace-check is now embtrace-sbom — same tool, new name "
        "(pipx install embtrace-sbom). This alias keeps working.[/yellow]"
    )
    main()
